...
curl -fsS "$BASE_URL/networking_tester/MACOS/interfaces-macos.sh" -o interfaces.sh || { echo "Failed to download interfaces-macos.sh"; exit 1; }
curl -fsS "$BASE_URL/networking_tester/MACOS/tests-macos.sh"      -o tests.sh      || { echo "Failed to download tests-macos.sh"; exit 1; }
curl -fsS "$BASE_URL/networking_tester/MACOS/output-macos.sh"     -o output.sh     || { echo "Failed to download output-macos.sh"; exit 1; }
